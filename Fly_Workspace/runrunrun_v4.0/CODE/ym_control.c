...
float Pitch_Zero=0.0,Pitch_error=0.00;//设置Pitch轴角度零点
float X_Balance_KP=1200,X_Balance_KI=3.5,X_Balance_KD=120;  // 平衡环PID  Pitch轴角度环PID参数 动量轮
float X_Velocity_KP=-40,X_Velocity_KI=-4;//动量轮电机速度环PI参数
...
void Balance(void)
{
    ...
    else if(MotorDutyA<-5000) MotorDutyA=-5000; // 动量轮电机限幅
    else if(MotorDutyA<-0) MotorDutyA -=500;    // 死区
    else if(MotorDutyA>0) MotorDutyA+=500;      // 死区
