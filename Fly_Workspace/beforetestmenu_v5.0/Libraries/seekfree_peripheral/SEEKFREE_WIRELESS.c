...
void wireless_uart_callback(void)
{
	while(uart_query(WIRELESS_UART, &wireless_rx_buffer))
	    {
	       if(wireless_rx_buffer=='a')
	       {
	           steer_duty-=40;
	           Set_steer(steer_duty);
	       }
	       else if(wireless_rx_buffer=='d')
	       {
	           steer_duty+=40;
	           Set_steer(steer_duty);
	       }
	       else if(wireless_rx_buffer=='w')
	       {
               MotorDutyB+=500;
	           gpio_toggle(LED0);    //打开蜂鸣器
	       }
	       else if(wireless_rx_buffer=='z')
	        {
	           steer_duty=800;
	           Set_steer(steer_duty);
	        }
	        else if(wireless_rx_buffer=='s')
	         {
	            MotorDutyB=0;
	            }
	    }
