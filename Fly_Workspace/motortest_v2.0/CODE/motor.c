...
//电机PWM
/*
#define Motor_back_Return ATOM0_CH4_P02_4
#define Motor_back_Go ATOM0_CH5_P02_5
#define Motor_balance_Return  ATOM1_CH6_P02_6
#define Motor_balance_Go  ATOM1_CH7_P02_7
*/
void motor_init(void)
{
    ...
    //初始化PWM引脚
    gtm_pwm_init(Motor_balance_Go, 17000, 0);
    gtm_pwm_init(Motor_balance_Return, 17000, 0);
    gtm_pwm_init(Motor_back_Go, 17000, 0);
    gtm_pwm_init(Motor_back_Return, 17000, 0);
...
{
    //对占空比限幅
    if(duty_m1 > GTM_ATOM0_PWM_DUTY_MAX)
        duty_m1 = GTM_ATOM0_PWM_DUTY_MAX;
    else if(duty_m1 < -GTM_ATOM0_PWM_DUTY_MAX)
        duty_m1 = -GTM_ATOM0_PWM_DUTY_MAX;

    if(duty_m2 > GTM_ATOM0_PWM_DUTY_MAX)
        duty_m2 = GTM_ATOM0_PWM_DUTY_MAX;
    else if(duty_m2 < -GTM_ATOM0_PWM_DUTY_MAX)
        duty_m2 = -GTM_ATOM0_PWM_DUTY_MAX;

    if(duty_m3 > GTM_ATOM0_PWM_DUTY_MAX)
        duty_m3 = GTM_ATOM0_PWM_DUTY_MAX;
    else if(duty_m3 < -GTM_ATOM0_PWM_DUTY_MAX)
        duty_m3 = -GTM_ATOM0_PWM_DUTY_MAX;

    if(duty_m4 > GTM_ATOM0_PWM_DUTY_MAX)
        duty_m4 = GTM_ATOM0_PWM_DUTY_MAX;
    else if(duty_m4 < -GTM_ATOM0_PWM_DUTY_MAX)
        duty_m4 = -GTM_ATOM0_PWM_DUTY_MAX;
